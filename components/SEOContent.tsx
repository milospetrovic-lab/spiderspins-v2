...
const sections = [
  {
    ...
    keywords: 'online slots · crypto slots · RTP slots',
    body: `At the centre of the SpiderSpins web sits a library of more than 500 slot games, drawn from the studios players already trust — RTG, Pragmatic, Betsoft, Nolimit City and rotating partners who feed us new silk every week. The spider does not care for noise. What we curate is math: RTPs published in the open, volatility ratings no one buries, and bonus-buy labels marked the moment you need them. Whether you chase a patient orb-weaver's spin (low volatility, near-daily small wins) or a widow's strike (extreme volatility, rare but decisive), the catalogue is sorted by the shape of the game, not by the size of a marketing budget. Every title loads inside the web — no redirects, no pop-ups, no unclosable overlays. Freespins from the Silk Thread promo trigger directly on featured games; comp points generated during each spin drop straight into your Spider Points balance as you play. When a new game joins the catalogue, it is flagged with a wine-red "NEW" thread for seven days, and the weekly Dispatch newsletter tells you which ones beat their advertised RTP in the first 48 hours of play. That's the difference between a casino that runs slots and a casino that reads them.`,
  },
  {
    id: 'tables',
